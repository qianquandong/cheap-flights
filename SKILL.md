...
2. **Run the flexibility levers.** Each is a separate check; report which ones actually moved the price and which didn't.
   ...
   - **Southwest is invisible to Google Flights — and therefore to `fli`.** For any US domestic route, check `southwest.com` in the browser pane separately or you will quote a wrong "cheapest". Bags fly free there, which often flips the ranking. **This check is untested in this skill** — say so when you run it.
